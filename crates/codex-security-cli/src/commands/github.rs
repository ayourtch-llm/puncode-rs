...
use codex_security::bulk_scan_discovery::{RepositoryNode, RepositoryPage, RepositorySource};
...
impl<T: GitHubTransport> RepositorySource for GitHubSource<T> {
    fn organizations(&self) -> codex_security::Result<Vec<String>> {
        let response = self
            .transport
            .get("/user/orgs?per_page=100")
            .map_err(codex_security::Error::codex_security)?;
    ...
    fn signed_in_account(&self) -> codex_security::Result<String> {
        self.transport
            .get("/user")
            .map_err(codex_security::Error::codex_security)?
            .get("login")
            ...
            .map(str::to_owned)
            .ok_or_else(|| {
                codex_security::Error::codex_security("GitHub did not report a signed-in account.")
            })
    }
    ...
    fn repositories(
        ...
        owner: &str,
        cursor: Option<&str>,
    ) -> codex_security::Result<RepositoryPage> {
        let response = self
            .transport
            .graphql(
                REPOSITORIES_QUERY,
        ...
        let connection = response
            ...
            .ok_or_else(|| {
                codex_security::Error::codex_security(
...
fn github_token(host: &str, environment: &ProcessEnvironment) -> Result<String, String> {
    ...
    let command = codex_security::trusted_executable::resolve_trusted_executable(
        "gh",
        environment,
        &protected_root,
    )
