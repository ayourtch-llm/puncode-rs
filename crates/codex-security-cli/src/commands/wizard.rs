...
use codex_security::bulk_scan_discovery::{
    ...
    if !prompt.is_interactive() {
        return Err(
            "Interactive repository selection requires a terminal. Provide a CSV with \
             'codex-security bulk-scan repositories.csv --output-dir ./security-scans'."
                .to_owned(),
        );
    }
...
mod tests {
    use super::*;
    use codex_security::bulk_scan_discovery::{RepositoryNode, RepositoryPage};
