...
use codex_security::runtime::{
    PluginPythonOptions, WorkbenchCommandOptions, bundled_plugin_root,
    codex_security_state_directory, resolve_plugin_python, run_workbench,
};
...
pub fn match_scans(arguments: &ScansMatchArgs, context: &HistoryContext) -> Result<String, String> {
    ...
    let agent = ProcessComparisonAgent::new(
        codex_security::runtime::resolve_codex_command(
            &context.environment,
            &context.current_directory,
        )
...
impl crate::commands::match_all::ComparisonStore for WorkbenchStore<'_> {
    fn save(
        ...
        before_scan_id: &str,
        after_scan_id: &str,
        matches: &codex_security::scan_comparison::ScanComparisonResult,
    ) -> Result<(), String> {
        workbench(
            &[
                "save-scan-comparison".to_owned(),
...
fn workbench(request: &[String], context: &HistoryContext) -> Result<Map<String, Value>, String> {
    ...
    environment.insert(
        ...
        codex_security_state_directory(&context.environment)
            .to_string_lossy()
            .into_owned(),
    );
    ...
    run_workbench(
        &WorkbenchCommandOptions {
            ...
            failure_message: Some("Could not read Codex Security scan history"),
        },
        &arguments,
    )
    .map_err(|error| error.to_string())
