...
use codex_security::config::{merged_codex_config, scan_model_configuration};
use codex_security::runtime::bundled_plugin_root;
...
pub fn build(
    ...
    let configuration = merged_codex_config(&codex_security::config::CodexSecurityConfig {
        codex_overrides: (!overrides.is_empty()).then_some(overrides),
        ..codex_security::config::CodexSecurityConfig::default()
    })
...
pub fn run(
    ...
    let codex = codex_security::runtime::resolve_codex_command(environment, current_directory)
        .map_err(|error| error.to_string())?;
