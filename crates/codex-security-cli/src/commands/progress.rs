...
impl ScanProgress {
    ...
    fn say(&self, message: &str) {
        ...
        let _ = writeln!(stderr, "codex-security: {message}");
        let _ = stderr.flush();
    }
}
...
impl ScanObserver for ScanProgress {
    ...
    fn on_reconnect(
        ...
        attempt: u32,
        max_attempts: u32,
        _details: Option<codex_security::api::ScanReconnectDetails>,
    ) {
            ...
            for raw in signals.forever() {
                ...
                match tracker.observe(signal, elapsed_ms) {
                    ...
                    SignalAction::Cancel => {
                        ...
                        let _ = writeln!(
                            stderr,
                            "codex-security: Stopping; press again to exit immediately."
                        );
                        let _ = stderr.flush();
                    }
