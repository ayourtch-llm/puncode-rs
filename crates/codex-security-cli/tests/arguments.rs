...
fn accepts_a_ref_with_its_target() {
    ...
    for arguments in [
        ...
        let (code, _, stderr) = run(&arguments);
        assert!(
            code == Some(0) || stderr.contains("codex-security:"),
...
fn reports_the_same_facts_as_json() {
    ...
    assert_eq!(report["nextStep"], "codex-security scan . --dry-run");
}
...
fn serves_the_protocol_over_standard_input() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_codex-security"))
        .arg("--mcp")
...
fn refuses_a_command_alongside_the_protocol() {
    let finished = Command::new(env!("CARGO_BIN_EXE_codex-security"))
        .args(["--mcp", "info"])
        .output()
        .expect("runs");
