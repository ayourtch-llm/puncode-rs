...
fn event(value: Value) -> puncode_security::Result<ThreadEvent> {
    Ok(serde_json::from_value(value).expect("event parses"))
}

/// The stream a successful scan produces.
fn successful_stream() -> Vec<puncode_security::Result<ThreadEvent>> {
    vec![
...
impl ScanObserver for Recorder {
    ...
    fn finalize(&mut self, usage: Option<&Value>) -> puncode_security::Result<Option<Value>> {
...
fn refuses_a_stream_that_ends_before_the_turn_completes() {
    ...
    assert_eq!(
        error.to_string(),
        "Puncode Security event stream ended before the turn completed."
    );
}
...
fn refuses_a_scan_that_never_reported_a_thread() {
    ...
    assert_eq!(
        error.to_string(),
        "Puncode Security did not report a thread ID."
    );
}
...
fn reports_a_cost_limit_stop_as_itself() {
    ...
    let expectation = expectation();
    let cancellation = ScanCancellation::new();
    let cost = puncode_security::ScanCost {
    ...
    cancellation.cancel_with(puncode_security::Error::scan_cost_limit_exceeded(
        10.0, cost, &scan,
    ));
    let mut recorder = Recorder::default();
...
fn propagates_a_failure_from_the_observer() {
    ...
    impl ScanObserver for FailingFinalize {
        fn finalize(&mut self, _usage: Option<&Value>) -> puncode_security::Result<Option<Value>> {
            Err(puncode_security::Error::puncode_security(
