...
use puncode_security::multiscan::{
...
use puncode_security::multiscan::{GitRunner, checkout_environment, checkout_revision};
...
impl GitRunner for FakeGit {
    fn run(
    ...
    ) -> puncode_security::Result<String> {
        ...
        {
            return Err(puncode_security::Error::puncode_security("git failed"));
        }
...
fn checkout(git: &FakeGit, host: Option<&str>) -> puncode_security::Result<()> {
...
fn checks_out_a_pinned_commit_from_a_real_repository() {
    use puncode_security::multiscan::ProcessGitRunner;
...
impl ScanRunner for FakeScanner {
    fn run(
        ...
        task: &MultiscanTask,
        scan_dir: &Path,
    ) -> puncode_security::Result<ScanOutcome> {
...

        {
            ...
            {
                *remaining -= 1;
                return Err(puncode_security::Error::puncode_security(
...
fn campaign(
    ...
    observer: &Progress,
) -> puncode_security::Result<puncode_security::multiscan::MultiscanResult> {
