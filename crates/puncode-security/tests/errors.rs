...
use puncode_security::ScanCost;
use puncode_security::error::{Error, ProtectedScanPathKind};
...
fn class_names_match_the_typescript_hierarchy() {
    let cases: Vec<(Error, &str)> = vec![
        (Error::puncode_security("x"), "PuncodeSecurityError"),
...
fn aggregates_several_failures_under_one_message() {
    let error = Error::aggregate(
        [
            ...
            Error::puncode_security("SYNTHETIC_PREPARATION_CLEANUP_FAILED"),
        ],
