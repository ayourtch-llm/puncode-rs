...
impl EventStream {
    ...
    fn finish(&mut self) -> Option<Result<ThreadEvent>> {
        ...
        Some(Err(Error::puncode_security(format!(
...
impl CodexThread for ProcessCodexThread {
    ...
    fn run_streamed(&mut self, input: &str) -> Result<EventStream> {
        ...
        let mut child = command.spawn().map_err(|error| {
            ...
            Error::puncode_security(message).with_source(error)
        })?;

        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| Error::puncode_security("Child process has no stdout"))?;
        let mut stdin = child
            .stdin
            .take()
            .ok_or_else(|| Error::puncode_security("Child process has no stdin"))?;
        let mut stderr = child
            .stderr
            .take()
            .ok_or_else(|| Error::puncode_security("Child process has no stderr"))?;
