...
//!
//! Ported from `PuncodeSecurity#run` in `src/api.ts`.
//!
...
use crate::runtime::{
    ...
    plugin_execution_environment, prepare_output_dir, prepare_persistent_scan_root,
    puncode_security_state_directory, require_model_safe_output_dir, resolve_plugin_python,
    run_workbench,
};
use crate::targets::{NormalizedTargetKind, ProcessEnvironment, repository_revision};

use super::client::{PuncodeSecurity, ScanOptions};
...
pub(crate) struct ScanRun<'a> {
    pub(crate) client: &'a PuncodeSecurity,
...
impl ScanRun<'_> {
    ...
    fn drive(
        ...
        let state_directory = puncode_security_state_directory(environment);
        ...
        if !credentials_available {
            return Err(Error::authentication_required(
                "No credentials were found. Run 'puncode-security login', use \
                 'puncode-security login --device-auth' on a remote or headless machine, or set \
        ...
        let workbench = WorkbenchCommandOptions {
            ...
            failure_message: Some("Could not save the Puncode Security scan"),
        };
    ...
    fn register(
        ...
        let recipe_json = serde_json::to_string(recipe).map_err(|error| {
            Error::puncode_security("Unable to serialize the Codex Security scan recipe")
                .with_source(error)
        })?;
    ...
    fn explain(&self, failure: Error, scan_dir: &Path) -> Error {
        ...
        if self.cancellation.is_cancelled() && !failure.is_scan_interrupted() {
            return Error::scan_interrupted(
                format!(
                    "Puncode Security scan was interrupted; partial output remains at {}.",
                    scan_dir.display()
                ),
                scan_dir,
            )
...
fn invalid_registration() -> Error {
    Error::puncode_security("The Codex Security workbench returned an invalid scan registration.")
}
...
impl ScanObserver for TrackingObserver<'_> {
    ...
    fn finalize(&mut self, usage: Option<&Value>) -> Result<Option<Value>> {
        let snapshot = self.tracker.stop(usage.cloned()).map_err(|error| {
            Error::puncode_security("Unable to read what the Codex Security scan spent")
                .with_source(error)
        })?;
    ...
    fn run_agent(
        ...
        if inputs.target().kind == Some(NormalizedTargetKind::Paths) {
            let path = runtime
                ...
                .join(format!(
                    "puncode-security-target-paths-{}.json",
    ...
    fn complete_scan(
        ...
        let cost_json = match cost {
            Some(cost) => Some(serde_json::to_string(cost).map_err(|error| {
                Error::puncode_security("Unable to serialize the Codex Security scan cost")
                    .with_source(error)
            })?),
            None => None,
        };
...
fn write_target_paths(path: &Path, paths: &[String]) -> Result<()> {
    ...
    let serialized = serde_json::to_string(paths)
        .map_err(|error| {
            Error::puncode_security("Unable to serialize the scan target paths").with_source(error)
        })?
        .replace('\u{85}', "\\u0085")
        .replace('\u{2028}', "\\u2028")
        .replace('\u{2029}', "\\u2029");
