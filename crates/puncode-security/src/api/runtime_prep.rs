...
//!
//! Ported from `PreparedRuntime` and `PuncodeSecurity#prepareRuntime` in
//! `src/api.ts`.
//!
...
use crate::config::{JsonObject, PuncodeSecurityConfig, merged_codex_config, write_codex_config};
use crate::error::{Error, Result};
use crate::runtime::{
    CodexCommand, CodexRunner, LocationCheck, PluginInstall, bootstrap_plugin,
    cleanup_sdk_directory, create_isolated_home, import_ambient_auth,
    puncode_security_state_directory, resolve_plugin_path,
};
...
pub struct RuntimePreparation<'a> {
    pub config: &'a PuncodeSecurityConfig,
...
impl<'a> RuntimePreparation<'a> {
    ...
    pub fn new(
        config: &'a PuncodeSecurityConfig,
...
fn prepare_within(
    ...
    *workspace = Some(bootstrap_workspace.clone());
    ...
    environment.insert(
        ...
        puncode_security_state_directory(preparation.environment)
            .to_string_lossy()
            .into_owned(),
    );
...
fn stop_if_cancelled(cancellation: &ScanCancellation) -> Result<()> {
    ...
    Err(Error::puncode_security(
        "Puncode Security runtime preparation was interrupted.",
    ))
}
...
mod tests {
    ...
    fn removes_both_directories_when_preparation_fails() {
        ...
        let leftovers: Vec<_> = std::fs::read_dir(harness.temporary.path())
            ...
            .filter(|name| name.to_string_lossy().starts_with("puncode-security-home-"))
            .collect();
        assert!(
            leftovers.is_empty(),
    ...
    fn reports_the_recorded_cancellation_reason() {
        ...
        harness
            .cancellation
            .cancel_with(Error::puncode_security("stopped for a reason"));
