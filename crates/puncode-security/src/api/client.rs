...
//!
//! Ported from `PuncodeSecurity` in `src/api.ts`.
//!
...
use crate::config::{PuncodeSecurityConfig, merged_codex_config, scan_model_configuration};
...
pub struct PuncodeSecurityMetadata {
    pub sdk: &'static str,
...
impl Default for PuncodeSecurityMetadata {
    fn default() -> Self {
        Self {
            sdk: "puncode-security",
            sdk_version: VERSION,
            executable: "codex",
...
pub type PrepareRuntimeFn =
    Box<dyn Fn(&PuncodeSecurityConfig, &ScanCancellation) -> Result<PreparedRuntime> + Send + Sync>;
...
pub struct PuncodeSecurity {
    config: PuncodeSecurityConfig,
    metadata: PuncodeSecurityMetadata,
...
impl PuncodeSecurity {
    ...
    pub fn with_dependencies(
        ...
        Self {
            config,
            metadata: PuncodeSecurityMetadata::default(),
            dependencies,
    ...
    #[must_use]
    pub fn config(&self) -> &PuncodeSecurityConfig {
        &self.config
    }

    #[must_use]
    pub fn metadata(&self) -> PuncodeSecurityMetadata {
        self.metadata
    }
    ...
    fn begin_operation(&self) -> Result<MutexGuard<'_, ()>> {
        ...
        match self.operation.try_lock() {
            Ok(guard) => Ok(guard),
            Err(TryLockError::WouldBlock) => Err(Error::puncode_security(
                "A Puncode Security operation is already in progress.",
            )),
...
fn closed() -> Error {
    Error::puncode_security("PuncodeSecurity is closed.")
}
...
mod tests {
    ...
    fn client_with(
        root: &Path,
        environment: &[(&str, &str)],
    ) -> (PuncodeSecurity, Arc<AtomicUsize>) {
    ...
    fn reports_what_produced_a_scan() {
        let metadata = PuncodeSecurity::new(PuncodeSecurityConfig::default()).metadata();

        assert_eq!(metadata.sdk, "puncode-security");
    ...
    fn stored_credentials(client: &PuncodeSecurity) -> bool {
        client
            .runtime
            .lock()
            .expect("runtime")
    ...
    fn local_only_client(environment: &[(&str, &str)]) -> PuncodeSecurity {
        let dependencies = ClientDependencies::default()
            .with_environment(
                environment
                    .iter()
    ...
    fn reports_the_configured_model_and_reasoning() {
        let root = TempDir::new().expect("root");
        let repository = repository(root.path());
        let config = PuncodeSecurityConfig {
        ...
        let client = PuncodeSecurity::with_dependencies(config, dependencies);
    ...
    fn refuses_configuration_that_takes_over_plugin_loading() {
        ...
        let client = PuncodeSecurity::with_dependencies(
            config,
