...
//! this port raises the base [`Error::puncode_security`] for all of them.
...
pub fn prepare_knowledge_base_in(
    ...
    let directory = tempfile::Builder::new()
        .prefix("puncode-security-knowledge-")
        .tempdir_in(root)
        .map_err(|error| {
...
fn extract_pdf(path: &Path, bytes: &[u8]) -> Result<String> {
    ...
    match extracted {
        Ok(Ok(text)) => Ok(text),
        Ok(Err(error)) => Err(Error::puncode_security(format!(
            "Cannot extract text from knowledge base PDF: {}",
            path.display()
        ))
        .with_source(error)),
        Err(_) => Err(Error::puncode_security(format!(
            "Cannot extract text from knowledge base PDF: {}",
            path.display()
        ))),
    }
