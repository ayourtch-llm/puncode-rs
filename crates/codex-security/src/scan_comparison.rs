...
pub fn match_scan_findings(
    ...
    let parsed: Value = serde_json::from_str(&response).map_err(|error| {
        Error::codex_security("Scan comparison returned invalid JSON.").with_source(error)
    })?;
...
pub fn comparison_prompt(input: &ScanComparisonInput) -> Result<String> {
    let serialized = serde_json::to_string(input).map_err(|error| {
        Error::codex_security("Unable to serialize the scan comparison input").with_source(error)
    })?;
    Ok([
...
fn invalid_match_result() -> Error {
    Error::codex_security("Scan comparison returned an invalid match result.")
}
...
impl ComparisonAgent for ProcessComparisonAgent {
    fn compare(&self, prompt: &str, options: &ScanComparisonOptions) -> Result<String> {
        ...
        let schema = tempfile::Builder::new()
            .prefix("codex-security-comparison-")
            .suffix(".json")
            .tempfile()
            .map_err(|error| {
                Error::codex_security("Unable to write the scan comparison schema")
                    .with_source(error)
            })?;
        serde_json::to_writer(&schema, &comparison_schema()).map_err(|error| {
            Error::codex_security("Unable to write the scan comparison schema").with_source(error)
        })?;
