...
pub fn parse_inventory(
    ...
    for fields in rows {
        ...
        if !seen.insert(id.to_lowercase()) {
            return Err(Error::codex_security("Multiscan task IDs must be unique."));
        }
...
pub fn normalize_repository(repository: &str, directory: &Path) -> Result<String> {
    ...
    let url = Url::parse(repository)
        .map_err(|_| Error::codex_security("Multiscan repository URL is invalid."))?;
...
pub fn build_github_credential_args(host: Option<&str>) -> Result<Vec<String>> {
    ...
    let invalid = || Error::codex_security("GitHub credential host is invalid.");
...
pub fn parse_csv_rows(source: &str) -> Result<Vec<Vec<String>>> {
    ...
    match errors.first() {
        Some(first) => Err(Error::codex_security(format!(
            "Multiscan CSV could not be parsed: {first}"
        ))),
        None => Ok(rows),
    }
...
pub fn ensure_manifest(path: &Path, tasks: &[MultiscanTask]) -> Result<()> {
    let expected = format!(
        ...
        .map_err(|error| {
            Error::codex_security("Unable to record the multiscan manifest").with_source(error)
        })?
    );
    ...
    match write_new_private(path, &expected) {
        ...
        Err(error) if error.to_string().contains("already exists") => {
            let existing = std::fs::read_to_string(path).map_err(|error| {
                Error::codex_security("Unable to read the multiscan manifest").with_source(error)
            })?;
...
pub fn read_receipts(path: &Path) -> Result<BTreeMap<String, MultiscanReceipt>> {
    ...
    if !contents.ends_with('\n') {
        ...
        std::fs::OpenOptions::new()
            ...
            .map_err(|error| {
                Error::codex_security("Unable to repair the multiscan ledger").with_source(error)
            })?;
    }
    ...
    for line in lines.iter().filter(|line| !line.is_empty()) {
        let receipt: MultiscanReceipt = serde_json::from_str(line).map_err(|error| {
            Error::codex_security("Multiscan ledger holds an unreadable receipt").with_source(error)
        })?;
...
pub fn append_receipt(path: &Path, receipt: &MultiscanReceipt) -> Result<()> {
    ...
    let line = format!(
        ...
        serde_json::to_string(receipt).map_err(|error| {
            Error::codex_security("Unable to record a multiscan receipt").with_source(error)
        })?
    );
...
fn write_new_private(path: &Path, contents: &str) -> Result<()> {
    ...
    let mut file = options.open(path).map_err(|error| {
        ...
        Error::codex_security(format!("{} {detail}", path.display())).with_source(error)
    })?;
    file.write_all(contents.as_bytes()).map_err(|error| {
        Error::codex_security(format!("Unable to write {}", path.display())).with_source(error)
    })
}
...
impl GitRunner for ProcessGitRunner {
    fn run(&self, arguments: &[String], environment: &ProcessEnvironment) -> Result<String> {
        ...
        .ok_or_else(|| Error::codex_security("Git is not available on a trusted PATH."))?;
        ...
        let output = std::process::Command::new(&command.executable)
            ...
            .output()
            .map_err(|error| {
                Error::codex_security(format!("Could not run git: {error}")).with_source(error)
            })?;
    ...
    std::thread::scope(|scope| {
        ...
        for handle in handles {
            let outcome = match handle.join() {
                Ok(outcome) => outcome,
                Err(_) => Err(Error::codex_security(
...
fn require_scope_inside(checkout: &Path, scope: &str) -> Result<()> {
    let escaped = || Error::codex_security("Multiscan scope escapes its repository.");
