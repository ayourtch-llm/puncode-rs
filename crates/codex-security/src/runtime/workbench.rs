...
#[must_use]
pub fn codex_security_state_directory(environment: &ProcessEnvironment) -> PathBuf {
...
pub fn run_workbench(
    ...
    let value: Value = serde_json::from_slice(&text).map_err(|error| {
        Error::codex_security("The Codex Security workbench returned invalid JSON.")
            .with_source(error)
    })?;
    match value {
        Value::Object(object) => Ok(object),
        _ => Err(Error::codex_security(
...
mod tests {
    ...
    fn uses_the_callers_failure_message() {
        ...
        options.failure_message = Some("Could not save the Codex Security scan");

        let error = run_stub(&options, &[]).expect_err("fails");

        assert_eq!(
            error.to_string(),
            "Could not save the Codex Security scan: boom"
        );
    }
    ...
    fn prefers_an_explicitly_configured_state_directory() {
        ...
        assert_eq!(
            codex_security_state_directory(&environment),
