...
fn reports_the_version_this_build_ships() {
    ...
    assert_eq!(
        manifest["version"],
        codex_security::version::BUNDLED_PLUGIN_VERSION
    );
}
...
fn marks_the_tree_complete_only_when_it_is() {
    ...
    assert_eq!(marker, codex_security::version::BUNDLED_PLUGIN_VERSION);
}
...
fn a_destination_can_be_checked_without_being_created() {
    use codex_security::runtime::validate_output_dir;
