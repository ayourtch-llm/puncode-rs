...
use codex_security::config::{
    CodexSecurityConfig, default_codex_config, merged_codex_config, scan_model_configuration,
    write_codex_config,
};
use serde_json::{Map, Value, json};
use tempfile::TempDir;

fn overrides(value: Value) -> CodexSecurityConfig {
    CodexSecurityConfig {
...
fn applies_defaults_when_there_are_no_overrides() {
    let merged = merged_codex_config(&CodexSecurityConfig::default()).expect("merge succeeds");
...
fn does_not_leak_overrides_between_merges() {
    ...
    let second = merged_codex_config(&CodexSecurityConfig::default()).expect("second merge");
...
fn reads_the_scan_model_configuration() {
    let config = merged_codex_config(&CodexSecurityConfig::default()).expect("merge");
