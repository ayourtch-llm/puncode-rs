...
use codex_security::api::collect_result;
use codex_security::contract::ScanExpectation;
use codex_security::result::TurnResultMetadata;
...
fn collect(scan: &Path) -> codex_security::Result<codex_security::ScanResult> {
...
fn names_every_missing_artifact_together() {
    ...
    assert_eq!(
        error.to_string(),
        "Codex Security scan completed without required artifacts: coverage.json, report.md"
    );
}
