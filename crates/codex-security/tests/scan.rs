//! End-to-end behavior tests for `CodexSecurity::run`.
//!
...
use codex_security::api::{
    ClientDependencies, CodexSecurity, PreparedRuntime, ScanCancellation, ScanObserver, ScanOptions,
};
use codex_security::codex::ProcessCodexClient;
use codex_security::config::CodexSecurityConfig;
use codex_security::cost::ScanCost;
use codex_security::runtime::{CodexCommand, PluginInstall};
...
fn plugin_tree(base: &Path) -> PathBuf {
    ...
    fs::write(
        ...
        json!({ "name": "codex-security", "version": PLUGIN_VERSION }).to_string(),
    )
...
impl Harness {
    ...
    fn with_artifacts(events: &str, write_artifacts: bool) -> Self {
        ...
        let mut harness = Self {
            ...
            environment,
            base,
            client: CodexSecurity::new(CodexSecurityConfig::default()),
        };
        harness.rewire();
        harness
    }
...
impl ScanObserver for Recorder {
    ...
    fn on_authentication(&mut self, _: codex_security::api::ScanAuthentication) {
...
fn removes_the_knowledge_base_once_the_scan_ends() {
    ...
    let leftovers: Vec<_> = fs::read_dir(harness._root.path())
        ...
        .filter(|name| name.starts_with("codex-security-knowledge-"))
        .collect();
    assert!(
        leftovers.is_empty(),
