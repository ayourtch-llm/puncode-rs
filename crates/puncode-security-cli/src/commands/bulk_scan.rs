...
use puncode_security::api::{IgnoreScanEvents, PuncodeSecurity, ScanCancellation, ScanOptions};
...
use puncode_security::targets::{ProcessEnvironment, ScanMode};
use serde_json::json;
...
fn config(arguments: &BulkScanArgs) -> Result<PuncodeSecurityConfig, String> {
    let overrides =
...
impl ScanRunner for CampaignScanner {
    fn run(
        ...
        let client = PuncodeSecurity::new(self.config.clone());
        let mut options = ScanOptions::new()
            .with_mode(task.mode)
            .with_output_dir(scan_dir.to_string_lossy().into_owned());
        if let Some(scope) = &task.scope {
            options = options.with_target(puncode_security::targets::ScanTarget::Paths(vec![
                scope.clone(),
            ]));
        }
